...
Make sure the `worker` component is connected to the `backend` network and the `server` component is connected to both the `frontend` and the `backend` networks.
Make sure the `AUTHENTIK_DB`, the `AUTHENTIK_USER` and the `AUTHENTIK_PASSWORD` environment variables are the same as in the postgres env file.
...
(See [the official proxy docs for traefik](https://docs.goauthentik.io/add-secure-apps/providers/proxy/server_traefik/))
...
Add the following label to the compose file of the app you want to put behind the proxy auth:
